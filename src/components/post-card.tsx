    ...
    <Link className="group" href={`/posts/${props.slug!.current}`}>
      <article className="flex flex-col-reverse gap-4 md:grid md:grid-cols-12 md:gap-0">
        ...
        <div className="md:col-span-5 md:w-full">
          <h2 className="relative text-2xl font-semibold text-pretty text-slate-800 transition-colors group-hover:text-blue-600">
            <span className="relative z-[1]">{title}</span>
            <span className="absolute inset-0 z-0 scale-75 rounded-lg bg-blue-100 opacity-0 transition-all group-hover:scale-x-105 group-hover:scale-y-110 group-hover:opacity-100" />
          </h2>
